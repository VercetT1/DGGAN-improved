    ...
    def __init__(self, n_node, node_emd_init, config):
        ...
        if node_emd_init:
            self.node_embedding_matrix = tf.compat.v1.get_variable(name = 'gen_node_embedding',
                                                       ...
                                                       initializer = tf.compat.v1.constant_initializer(self.node_emd_init),
                                                       trainable = True)
        else:
            self.node_embedding_matrix = tf.compat.v1.get_variable(name = 'gen_node_embedding',
        ...
        self.node_embedding = tf.nn.embedding_lookup(params=self.node_embedding_matrix, ids=self.node_ids)

        _noise_embedding= []
        for i in range(2):
            _noise_embedding.append(tf.reshape(tf.nn.embedding_lookup(params=self.noise_embedding, ids=tf.constant([i])), 
                                               [-1, self.emd_dim]))
        _dis_node_embedding = []
        for i in range(2):
            _dis_node_embedding.append(tf.reshape(tf.nn.embedding_lookup(params=self.dis_node_embedding, ids=tf.constant([i])), 
                                                  [-1, self.emd_dim]))
        ...
        for i in range(2):
            ...
            _score[i] = tf.reduce_sum(input_tensor=tf.multiply(_dis_node_embedding[i], _fake_node_embedding), axis=1)

            _neg_loss[i] = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(\
