...
class LinkPrediction():
    ...
    def evaluate(self, embedding_matrix):
        ...
        for i in range(len(selected_embeddings[0])):
            emb1 = selected_embeddings[0][i]
            emb2 = selected_embeddings[1][random.randrange(number_of_nodes)]
            distance = manhattan_distance(emb1,emb2)
        ...
        for s, t in train_links:
          one = embedding_matrix[0][s]
          two=embedding_matrix[1][t]
        ...
        percentile_5 = np.percentile(sorted_distances, 40)
        ...
        for i in range(len(test_y)):
              ...
              with open('total_result.txt', 'w') as outfile:
                  ...
                  outfile.write('\n')
                  #outfile.write('Average: '+str((min+max)/len(self.links)))
